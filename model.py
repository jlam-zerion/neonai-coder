...
class Bedrock(LLM):
    """
    Bedrock LLM implementation using AWS Bedrock to access Claude
    """
    def __init__(self, api_key: Optional[str] = None, call_limit: int = 50, model_call_cb: Optional[Callable[[int, int], None]] = None):
        """
        Initialize Bedrock model for Claude access
        """
        super().__init__(None, call_limit, model_call_cb)
    ...
    def __invoke_model(self, config:dict, messages:list, verbose:bool) -> str:
        import json
        ...
        try:
            streaming_response = self.client.invoke_model_with_response_stream(modelId=model_id, body=request)
            model_response = ""
            for event in streaming_response["body"]:
                chunk = json.loads(event["chunk"]["bytes"])
                if (chunk["type"] == "content_block_delta"):
                    response = chunk["delta"].get("text", "")
                    print(response, end="")
    ...
    def get_model_response(self, 
        ...
        config = {
            'max_tokens': claude_config.get('max_tokens', 4096),
            'temperature': claude_config.get('temperature', 0.5),
            'top_p': claude_config.get('top_p', 0.999),
            'top_k': claude_config.get('top_k', 250),
            'stop_sequences': claude_config.get('stop_sequences', [])
        }
                ...
                for file_path, content in (tqdm(file_entries, desc="Summarizing file contents", unit="file", disable=not verbose) if verbose else file_entries):
                    ...
                    if cache_entry is not None and 'summary' in cache_entry:
                        ...
                        try:
                            ...
                            summary_response = self.__invoke_model(config, [{"role": "user", "content": summary_prompt}], verbose=verbose)
                            self._call_count += 1
        ...
        # Request response from Claude
        try:
            full_response = self.__invoke_model(config, messages, verbose=verbose)
        except Exception as e:
            log_message(f"Error in Claude API call: {e}")
            return ""
...
def get_model(api_key: Optional[str] = None, model_type: str = "gemini", call_limit: int = 50, model_call_cb: Optional[Callable[[int, int], None]] = None) -> LLM:
    ...
    if model_type == "gemini":
        ...
        return Bedrock(None, call_limit=call_limit, model_call_cb=model_call_cb)
    else:
